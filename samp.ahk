...
CalcScreenCoordX(Coords, ScreenRange:=50) {
	...
	if(frX := (Coords[3] * readFloat(hGTA, dwM + 8*4) + Coords[2] * readFloat(hGTA, dwM + 4*4) + Coords[1] * readFloat(hGTA, dwM + 0*4) + readFloat(hGTA, dwM + 12*4))*(fRecip * dwLenX) >= (((dwLenX//2)-ScreenRange)+(VAR_RADIANT*POS_OFFSET)) && frX <= (((dwLenX//2)+ScreenRange)+(VAR_RADIANT*POS_OFFSET))){
		return True
	}
}
...
CenterCheckpoint(Coords, ScreenRange:=50) {
	if(!checkHandles() || !isObject(Coords))
		return false
...
GetCamInfo(Flag:="D")
{
    If(!checkHandles())
        return false
	if(Flag == "R"){
		AX := (readFloat(hGTA, ADDR_CAMERA_ANGLE_X)*(VAR_RADIANT*180))?(-1):(AX<0)?(AX+=(VAR_RADIANT*360)):(AX)
		AY := (readFloat(hGTA, ADDR_CAMERA_ANGLE_Y)*(VAR_RADIANT*180))?(-1):(AY<0)?(AY+=(VAR_RADIANT*360)):(AY)
		AZ := (readFloat(hGTA, ADDR_CAMERA_ANGLE_Z)*(VAR_RADIANT*180))?(-1):(AZ<0)?(AZ+=(VAR_RADIANT*360)):(AZ)	
	}
	else if(Flag == "D"){
	...
	return {AX:AX,AY:AY,AZ:AZ,PX:readFloat(hGTA, ADDR_CAMERA_POS_X),PY:readFloat(hGTA, ADDR_CAMERA_POS_Y)}
}
...
CoordsFromRedmarker2D() {
    if(!checkhandles())
        return false
    return [ readFloat(hGTA, 0xC7DEC8), readFloat(hGTA, 0xC7DECC)]
}
; ##### %#Positionsbestimmung  #####
...
InRangeOf(_Pos, _posRadius) {
	GetPlayerPos(_PlayerX, _PlayerY, _PlayerZ)
	X := _Pos[1] -_PlayerX
	Y := _Pos[2] -_PlayerY
	Z := _Pos[3] -_PlayerZ
	if(((X < _posRadius) && (X > -_posRadius)) && ((Y < _posRadius) && (Y > -_posRadius)) && ((Z < _posRadius) && (Z > -_posRadius)))
